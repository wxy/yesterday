 ...
 */
export function getConfigDefs(): Record<string, ConfigDefinitionItem> {
  return {
    // ===== 常规设置 =====
    'requestTimeout': {
      type: Number,
    ...
    'language': {
      ...
      ui: {
        type: 'select',
        label: _('config_language_label', '界面语言'),
        description: _('config_language_desc', '选择扩展界面语言，自动为跟随浏览器。'),
        section: _('config_section_general', '常规设置'),
        options: [
          { value: 'auto', label: _('config_language_auto', '自动（跟随浏览器）') },
          { value: 'zh_CN', label: _('config_language_zh_CN', '简体中文') },
          { value: 'en', label: _('config_language_en', 'English') }
        ] // 默认兜底，防止为空
 ...
 * 配置定义类型
 */
export type ConfigDefs = ReturnType<typeof getConfigDefs>;
...
export type ConfigSchema = {
  [K in keyof ConfigDefs]: ReturnType<ConfigDefs[K]['type']>;
};
